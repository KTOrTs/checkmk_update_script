...
create_site_backup() {
    ...
    debug_log "Estimated site size (du -sk ${CHECKMK_DIR}): ${site_size_kb:-unknown} KB (uncompressed)"
    debug_log "Available space in backup target: ${available_kb:-unknown} KB"

    echo -e "${TEXT_YELLOW}Estimated site size (du -sk ${CHECKMK_DIR}): ${site_size_mb_display} MB (uncompressed)${TEXT_RESET}"
    ...
    echo -e "${TEXT_YELLOW}Creating backup for site ${CHECKMK_SITE}... (backup archive is compressed, final size will be smaller than the uncompressed estimate)${TEXT_RESET}"
    ...
    while kill -0 "$BACKUP_PID" 2>/dev/null; do
        ...
        if [[ -n "$site_size_kb" && "$site_size_mb_display" != "unknown" ]]; then
            local progress_percent
            progress_percent=$(awk -v cur="$current_mb" -v est="$site_size_mb_display" 'BEGIN { if (est>0) printf "%.2f", (cur/est)*100; else print "0.00" }')
            printf "\r${TEXT_BLUE}Backup progress: %s MB (compressed) | %s%% of estimated %s MB (uncompressed)${TEXT_RESET}" "${current_mb}" "${progress_percent}" "${site_size_mb_display}"
        else
            printf "\r${TEXT_BLUE}Backup progress: current %s MB (compressed)${TEXT_RESET}" "${current_mb}"
        fi
        sleep 2
    done
    ...
    if [ $BACKUP_EXIT -ne 0 ]; then
        ...
        if [ -f "$backup_file" ]; then
            ...
            if [[ -n "$site_size_kb" && "$site_size_mb_display" != "unknown" ]]; then
                ...
                compression_ratio=$(awk -v final_bytes="$final_bytes" -v site_kb="$site_size_kb" 'BEGIN { if (site_kb>0) printf "%.2f", (final_bytes/1024/site_kb)*100; else print "N/A" }')
                debug_log "Backup compression: ${compression_ratio}% of estimated uncompressed size"
                echo -e "${TEXT_BLUE}Backup finished: ${final_mb} MB on disk (~${compression_ratio}% of estimated uncompressed size)${TEXT_RESET}"
            else
                echo -e "${TEXT_BLUE}Backup finished: ${final_mb} MB on disk${TEXT_RESET}"
            fi
        fi
